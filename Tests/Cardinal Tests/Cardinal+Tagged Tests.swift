...
extension Cardinal.Tagged.Unit {
    ...
    func `construction from UInt`() {
        let users = Tagged.Tagged<UserCount, Cardinal>(100 as UInt)
        #expect(users.underlying == Cardinal(100))
    }

    @Test
    func `construction from int success`() throws(Cardinal.Error) {
        let users = Tagged.Tagged<UserCount, Cardinal>(42)
        #expect(users.underlying == Cardinal(42))
    }

    @Test
    func `add saturating`() {
        let max = Tagged.Tagged<UserCount, Cardinal>(UInt.max)
        let result = max.add.saturating(Tagged.Tagged<UserCount, Cardinal>(1 as UInt))
        #expect(result.underlying == Cardinal(UInt.max))
    }

    @Test
    func `add exact succeeds`() throws(Cardinal.Error) {
        let a = Tagged.Tagged<UserCount, Cardinal>(5 as UInt)
        let b = Tagged.Tagged<UserCount, Cardinal>(3 as UInt)
    ...
    func `different tags are distinct types`() {
        let users = Tagged.Tagged<UserCount, Cardinal>(10 as UInt)
        let inboxItems = Tagged.Tagged<InboxCount, Cardinal>(10 as UInt)
...
extension Cardinal.Tagged.`Edge Case` {
    ...
    func `construction from int fails for negative`() {
        #expect(throws: Cardinal.Error.negativeSource(-3)) {
            try Tagged.Tagged<UserCount, Cardinal>(Int(-3))
        }
    }
    ...
    func `add exact throws on overflow`() {
        let max = Tagged.Tagged<UserCount, Cardinal>(UInt.max)
        let one = Tagged.Tagged<UserCount, Cardinal>(1 as UInt)
    ...
    func `subtract saturating underflow clamps to zero`() {
        let a = Tagged.Tagged<UserCount, Cardinal>(3 as UInt)
        let b = Tagged.Tagged<UserCount, Cardinal>(5 as UInt)
        let result = a.subtract.saturating(b)
    ...
    func `int conversion throws on overflow`() {
        let users = Tagged.Tagged<UserCount, Cardinal>(UInt.max)
        #expect(throws: Cardinal.Error.overflow) {
            try Int(users)
        }
    }
...
extension Cardinal.Tagged.Integration {
    ...
    func `int conversion success`() throws(Cardinal.Error) {
        let users = Tagged.Tagged<UserCount, Cardinal>(42 as UInt)
